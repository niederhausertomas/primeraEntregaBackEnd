...
class ProductManager{
    ...
    async #getIndex(id){
        ...
        if (product != "Id not found") index=this.products.indexOf(product) 
        else return; 
        return index
    }
    ...
    getProductById = async (id) => {
        await this.getProducts()
        return this.products.find(product => product.id == id) || "Id not found";
    ...
    deleteProductById = async (id) => {
        console.log(await this.#getIndex(id))
        const index = await this.#getIndex(id)
        if (index>=0) return (this.products.splice(index, 1), await fs.promises.writeFile(this.path, JSON.stringify(this.products)),{message: "Success"})
